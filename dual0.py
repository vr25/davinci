...
dataset = load_dataset("tommasobonomo/sem_augmented_fever_nli", split="test")  # or "scifact"
...
for example in dataset.select(range(dataset_len)):  # small batch for demo
    ...
    evidence = " ".join(example['premise']) if isinstance(example['premise'], list) else example['premise']
